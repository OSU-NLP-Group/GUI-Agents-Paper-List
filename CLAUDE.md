...
The pipeline first runs `scripts/normalize_institutions.py --write`, which
rewrites institution names onto their canonical form. It then runs
`scripts/regen.py`, which:
1. Loads `papers.yaml` and `adjacent.yaml`.
2. Fills in `bibtex` for any entry that lacks one, leaving existing values alone.
3. Sorts entries newest-first, writes the YAML files back canonicalised (sorted, ISO date format).
4. Renders `README.md` directly from `readme_template/template.md` (no intermediate fragment files).
5. Emits `readme_template/statistics/{quarterly_trend,keyword_bar_chart}.png`.
...
│   ├── normalize_institutions.py     ← canonicalise institution names
│   ├── regen.py                 ← single-pass: fill BibTeX + sort YAML + render README + emit charts
...
- title: "Paper title (preserve official capitalization)"
  ...
  bibtex: |                                     # generated by regen.py if absent
...
**Link.** One canonical URL, with every other known URL in `sources:`. Prefer the arXiv abstract page when the paper is on arXiv, since it keeps `arxiv_id` derivable and matches the identifier `paper_db` keys its records on. Fall back to `openreview` → `publisher_page` → `homepage` for papers that never went to arXiv. A project page belongs in `sources.homepage`, not here.

**Authors.** Include all of them.

**Institutions.** Common abbreviations preferred (OSU, MIT, CMU). Use `Unknown` only if no source specifies one. `scripts/normalize_institutions.py` rewrites recognised names onto one canonical spelling on every pipeline run; it only touches names matching a rule, so an institution it does not know keeps whatever you wrote. Add a rule there rather than hand-editing entries when a name shows up under two spellings.
